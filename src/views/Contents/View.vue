        ...
        <div id="c-container">
            <article class="boardview">
                <div class="header">
                    ...
                    <h2>{{detail.title}}</h2>
                    <div class="info">
                        <p class="tag"><span v-for="(tag, index) in detail.relationTag" :key="index"><Tag :tag="tag" :gubun="tagGubun" /></span></p>
            ...
            <keep-alive>
                <Pvnt :prev="prev" :next="next" :type="type" />
            </keep-alive>
            <div class="btnArea">
                <a @click="goList" class="btnList">목록으로</a>
            </div>
        </div><!-- //container -->
    </div><!-- //wrapper -->
...
export default {
    ...
    methods: {
         initPage(query){
            ...
            this.menuIndex = this.type.substring(this.type.length, this.type.length-1)
        ...
        goList(){
            this.$router.push({
                name:'Contents', 
                query:{type:this.type, cate:this.cateSeq}
            })
        }
    }, 
}
...
.boardview {
    .header {padding:0 20px; margin:0 -20px 40px;
        .head {position:relative;
            ...
            .btns {float:right; padding-right:50px;
