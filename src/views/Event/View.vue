<template>
    <div id="c-container" :class="{fixed:fixed}">
        ...
        <meta property="og:image" :content="thumb(detail.thumbnail)">
        ...
        <article class="viewArea">
            <header>
                <h2>{{detail.title}}</h2>
                <div class="info">
                    <p class="eventDate">이벤트 기간 <span class="colorA">{{formatDate(detail.startDate)}} ~ <template v-if="(detail.productExhaustionYn == 'Y')">제품소진시</template><template v-else>{{formatDate(detail.endDate)}}</template></span></p>
            ...
            <CommentForm :eventSeq="seq" @addComment="addComment" />
...
export default {
    ...
    methods: {
         initPage(query){
            ...
            axios.get('/event/ing/detail.do?seq=' + this.seq).then(res => {
        ...
        addComment(payload) {
            axios.post('/event/comment/ins.do', qs.stringify(payload))
            .then(res => {
                this.goCommentList()
            })
            .catch(err => {
...
.viewArea {
    header {padding:0 20px 40px; margin:0 -20px; border-bottom:2px solid #212121;
        ...
        .info {position:relative; text-align:right;;
            ...
            .count {position:relative; display:inline-block; padding-left:25px; margin-right:30px; line-height:1em;
